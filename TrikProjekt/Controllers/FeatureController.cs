namespace TrikProjekt56.Controllers
{
    ...
    public class FeatureController : Controller
    {
        ...
        public IActionResult Index(string sortExpression = "", string SearchText = "", int pageIndex = 1, int pageSize = 5)
        {
            ...
            Pagination<DistFeature> items = _repo.GetItems(sortModel.SortedProperty, sortModel.SortedOrder, SearchText, pageIndex, pageSize);
        ...
        private readonly IFeature _repo;

        public FeatureController(IFeature repo)
        {
            _repo = repo;
        }
        ...
        {
            DistFeature item = new DistFeature();
            TempData.Keep();
            return View(item);
        }

        [HttpPost]
        public IActionResult Create(DistFeature item)
        {
            bool myBool = false;
            string errMessage = "";
            try
        ...
        {
            DistFeature item = _repo.GetItem(id);
            TempData.Keep();
            return View(item);
        }
        ...
        [HttpPost]
        public IActionResult Edit(DistFeature item)
        {
            bool myBool = false;
            string errMessage = "";
            try
        ...
        [HttpPost]
        public IActionResult Delete(DistFeature item)
        {
            try
            {
