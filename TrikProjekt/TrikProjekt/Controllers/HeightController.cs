namespace TrikProjekt56.Controllers
{
    ...
    public class HeightController : Controller
    {
        ...
        public IActionResult Index(string sortExpression = "", string SearchText = "", int pageIndex = 1, int pageSize = 5)
        {
            ...
            Pagination<Height> items = _repo.GetItems(sortModel.SortedProperty, sortModel.SortedOrder, SearchText, pageIndex, pageSize);
        ...
        private readonly IHeight _repo;

        public HeightController(IHeight repo)
        {
            _repo = repo;
        }
        ...
        [HttpPost]
        public IActionResult Create(Height item)
        {
            int currentPage = 1;
        ...
        [HttpPost]
        public IActionResult Edit(Height item)
        {
            int pageSize = 5;
            if (TempData["PageSize"] != null)
            {
        ...
        [HttpPost]
        public IActionResult Delete(Height item)
        {
            int pageSize = 5;
            if (TempData["PageSize"] != null)
            {
